 ...
 * @param fields
 */
const handleAdd = async (fields: API.ArticleCategory) => {
 ...
 * @param fields
 */
const handleUpdate = async (fields: API.ArticleCategory) => {
 ...
 * @param selectedRows
 */
const handleRemove = async (selectedRows: API.ArticleCategory[]) => {
...
const Role: React.FC = () => {
  ...
  const [currentRow, setCurrentRow] = useState<API.ArticleCategory>();
  const [selectedRowsState, setSelectedRows] = useState<API.ArticleCategory[]>([]);
  const [readOnly, setReadOnly] = useState<boolean>(false);

  const [categoryType, setCategoryType] = useState<any>({});
  React.useEffect(() => {
    valueEnum("article_category_type").then(res => {
      setCategoryType(res);
    });
  }, []);
  ...
  const columns: ProColumns<API.ArticleCategory>[] = [
    {
    ...
    {
      title: <FormattedMessage id="pages.category.categoryId"/>,
      dataIndex: 'categoryId',
    },
    {
      title: <FormattedMessage id="pages.category.name"/>,
      dataIndex: 'name',
    ...
    {
      title: <FormattedMessage id="pages.category.type"/>,
      dataIndex: 'type',
      valueEnum: categoryType,
    },
    {
      title: <FormattedMessage id="pages.category.color"/>,
      dataIndex: 'color',
      renderText: (val: string) => (
        <Tag color={val} bordered={false}>{val}</Tag>
      ),
    },
    {
  ...
  return (
    <PageContainer>
      <ProTable<API.ArticleCategory, API.ArticleCategoryQuery>
      ...
      <SaveForm
        ...
        onSubmit={async (value) => {
          const success = await handleAdd(value as API.ArticleCategory);
          if (success) {
